...
//
// Parked on globalThis deliberately. Next bundles route handlers and
// server-rendered pages separately, so a plain module-level array gives
// /api/lead-update and /admin two different arrays: the API accepts a write and
// the admin board still reports nothing worked. One process, one array.
// Irrelevant once GOOGLE_SHEET_ID is set, since everything then reads the Sheet.
const memoryUpdates = (globalThis.__fridoMemoryUpdates ??= []);
