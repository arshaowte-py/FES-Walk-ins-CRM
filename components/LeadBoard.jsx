...
export default function LeadBoard({ storeName, leads, summary, previewMode }) {
  ...
  useEffect(() => {
    ...
    const savedSort = window.localStorage.getItem("frido_sort");
    if (savedSort && SORTS.some((s) => s.key === savedSort)) setSort(savedSort);
  }, []);
  ...
  // Remember the order this store prefers, so it survives a refresh.
  function saveSort(v) {
    setSort(v);
    window.localStorage.setItem("frido_sort", v);
  }
  ...
  const filtered = useMemo(() => {
    ...
    // Sort after filtering — same result, but over a much smaller list.
    return sortLeads(matches, sort);
